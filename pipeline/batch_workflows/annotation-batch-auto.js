export const meta = {
  name: 'annotation-batch-auto',
  description: 'Corroborate a packet of CNMH events (passed via args) — FIND -> adversarial VERIFY; draft cited narrative',
  phases: [
...
// PACKET is supplied per-run via Workflow `args` (an array of coded packets),
// or injected as a literal by pipeline/make_batch.py (UTF-8, no console round-trip).
const PACKET = Array.isArray(args) ? args : []
if (!PACKET.length) throw new Error('annotation-batch-auto: empty packet — pass the batch slice via args')
