...
// Fire ramp + timing knobs live in fireRamp.ts (no deck.gl dependency, so the
// landing page can quote the same ramp). Re-exported here: this module stays the
// import site every existing caller already uses.
export { FIRE_DEFAULTS, RAMP_DEFAULTS, rampStopToVec4, type FireKnobs } from './fireRamp';
import { FIRE_DEFAULTS, RAMP_DEFAULTS, rampStopToVec4, type FireKnobs } from './fireRamp';
